...
from . import sid_frechet_inception_distance_edm2 as frechet_inception_distance
#from . import sid_kernel_inception_distance_edm2 as kernel_inception_distance
#from . import sid_precision_recall_edm2 as precision_recall
from . import perceptual_path_length
#from . import sid_inception_score_edm2 as inception_score
...
def fid50k_full(opts):
    ...
    if detector_dino is None:
        ...
        fid,fid_dino = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=50000,batch_size=opts.batch_size,batch_gen=opts.batch_gen)
...
def fid_test(opts):
    ...
    detector_dino = getattr(opts, 'detector_dino', None)
    if detector_dino is None:
        fid = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=512,batch_size=opts.batch_size,batch_gen=opts.batch_gen)
        return dict(fid50k_full=fid)
    else:
        fid,fid_dino = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=512,batch_size=opts.batch_size,batch_gen=opts.batch_gen)
        return dict(fid50k_full=fid,fid_dino=fid_dino)
